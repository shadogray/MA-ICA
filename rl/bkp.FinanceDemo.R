...
finDemo <- function(sSteps = 10, tSteps = 20, baseX = 100,
                    ...
                    ica = T, numSigs = 2, numBase = 7, numOrig = 3,
                    fErr.v0 = baseX, fErr.base = baseX, fErr.a = 1, fs.pred = 'S', ...) {
  ...
  x$W.init <- c((corrr::correlate(xb))$FS[1:x$xSigColNum],.5)
  x$W <- x$W.init
  x$W.prev <- x$W
  ...
  x$Acts <- tibble()
  for (n in x$xSigColNames) x$Acts <- crossing(x$Acts, !!n := c(-1, 0, 1))
  x$Acts$SC <- 0
  ...
  x$log <- tibble(lid=integer(), time=numeric(), i=integer(), t=integer(), sid=integer(), aid=integer(), Qpo=numeric())
...
#'    geom_vline(aes(xintercept=-1),linetype='longdash')
#' fe <- tibble(e=seq(-200,200,.01),y=fErr(x, seq(-200,200,.01))); ggplot(fe)+geom_line(aes(e,y))
...
calculateDx <- function(x, SValues, A, SValues.prev = x$SValues.prev, t = x$t, v0 = x$baseX, base = x$baseX, a = x$numSigs, alpha = x$alpha) {
  #R.estimate <- estimateR(x, SValues, A, SValues.prev)
  #Q <- x$Q[A$aid,x$xCols]
  #((R.estimate / x$baseX) + Q)
  #R.estimate
  #s <- getState(x, SValues)
  #s.prev <- getState(x, SValues.prev)
  #t(apply(s[,x$vCols], 1, `-`, as.numeric(s.prev[,x$vCols])))
  #R.estimate * t(apply(SValues, 1, `-`, as.numeric(SValues.prev))) / x$baseX

  w.s <- sqrt(sum((x$W*SValues)^2))
  Fs <- x$FS$Si[t]

  signDiff <- replace(sign(w.s - Fs), (w.s - Fs)==0, 1)

  dx <- ifelse(w.s == 0,
    1,
    ifelse(abs(w.s-Fs) > 1,
      - v0/a/base * signDiff * 1/w.s,
      - v0/a/base * (w.s - Fs) * 1/w.s
    )
  ) * (x$W*SValues^2)
...
penalize <- function(w, k=.4, d=2, a=1.7) {
  ...
  if (!failed && !A$terminal) {
    if (is.null(Anext)) {
      ...
      if (!any(is.na(m$SValues))) {
        ...
        Ze <- lambda * x$Z + (1 - alpha*gamma*lambda*sum(x$Z*dQ/x$baseX)) * dQ / x$baseX
        maxZe <- max(abs(Ze))
        x$Z <- Ze * penalize(maxZe, a=1, d=5)/maxZe
        if (max(abs(Ze)) > 1) {
        ...
        We <- W + alpha * (dw.dqz - dw.qz) #/ 10 # not too much volatility
        maxWe <- max(abs(We))
        W <- We * penalize(maxWe)/maxWe
        if (max(abs(We)) > 1) {
          #W <- W / max(c(1,abs(as.numeric(W))))
          print(paste0('recalibrated: W=',toString(W),', We=',toString(We)))
        }
