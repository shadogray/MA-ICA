...
takeAction <- function(A, myEnv = finDemo) {
  ...
  if (nlog) print(paste0('takeAction Res: resA=', toString(list(resEnv$resA))))
  return(resEnv)
}
...
control <- function(currAct, R, nextAoS, myEnv = finDemo, type = 'Q') {
  ...
  W <- as.numeric(myEnv$W) # [currAct$aid,])
  #W <- as.numeric(myEnv$W)
  if (F&&deblog) {
    if (!is.null(nextAoS)) print(tibble(w=toString(W), wn=toString(W), R = toString(R), alph = alpha, gam = gamma, t = myEnv$t))
    if (is.null(nextAoS)) print(tibble(w=toString(W), R = toString(R), alph = alpha, gam = gamma, t = myEnv$t), n=Inf, width=Inf)
    print(currAct %>% bind_rows(nextAoS, getStateAction(myEnv, currAct$sid, currAct$aid)), n=Inf, width=Inf)
    print(paste('control[',myEnv$t,']: W=', toString(W)))
  }
...
learn <- function(myEnv = finDemo, numIter = 100, type = 'SARSA') {
  ...
  #flog.appender(appender.file(paste0('run_',myEnv$numSigs,'.log')))
  #flog.threshold(DEBUG)    # TRACE, DEBUG, INFO, WARN, ERROR, FATAL
  ...
  for (i in iStart:numIter) {
    print(paste0('iter:',i,'time:',Sys.time()))
    myEnv$Iterations <- i
    myEnv$i <- i
    ...
    if (type == 'SARSA') {
      ...
      while(TRUE) {
        ...
        #nextState <- move(myEnv, currState, currAct)
        nextState <- resA$S
    ...
    if (i%%(100/myEnv$numSigs) == 0) save(myEnv, tracks, file = saveFileName)


    meanDiff <- (meanDiff*(i-1) + abs(lastSumR - myEnv$SumR)) / i
    if (myEnv$SumR > 0 && abs(meanDiff/myEnv$SumR) < deltaSumR) {
      print(paste('END: SumR:',toString(myEnv$SumR),'diffSumR:',toString(lastSumR - myEnv$SumR),'meanDiff:',toString(meanDiff)))
      save(myEnv, tracks, file = saveFileName)
      #break
    } else {
