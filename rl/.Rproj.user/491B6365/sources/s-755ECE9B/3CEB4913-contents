...
evalLogSuccess <- function(res, env, iter, tag) {
  ...
  g.sumMean <- mean(pull(trs %>% filter(t==20),res.SumR))
  g.sdMean <- sd(pull(trs %>% filter(t==20), res.SumR))
  ...
  res$g.sumMean <- g.sumMean
  res$g.sdMean <- g.sdMean
...
if (T) {
  ...
  tryCatch({
    succ <- tibble(numOrig=numeric(),numSigs=numeric(),iter=numeric(),
                   ...
                   g.sumMean=numeric(), g.sdMean=numeric()
    )
      ...
      try({
        ...
        for (i in 1:length(result$myEnvs)) {
          ...
          succ <- add_row(succ, numOrig=e$numOrig, numSigs=e$numSigs, iter=i,
                          ...
                          g.sumMean=res$g.sumMean, g.sdMean=res$g.sdMean)
        }
