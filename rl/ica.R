...
for (i in 1:(nSig-1)) {
  data <- data %>%
    bind_cols(tibble(x = i/nSig*x, y = (1-i/nSig)*y))
}
...
####### Run ICA 3 signals
mrand <- matrix(rnorm(dim(data)[1]*dim(data)[2])*.0001, nrow=dim(data)[1])
ica <- fastICA::fastICA(data+mrand, n.comp = 3, row.norm = T)
summary(ica)
ggplot(as_tibble(ica$S) %>% rowid_to_column('t') %>%
         gather(key,value,-t), aes(t,value, color=key))+
  geom_line()+
  ...
  ylab('Amplitude')+
  ggtitle('Independent Components ')
ggsave('ICA_Demo_3ICASignals.png')
